        ...
        private string BuildRemarks()
        {
            ...
            for (int i = 0; i < DgvStockReceipt.Rows.Count - 1; i++)
            {
                Remarks += DgvStockReceipt.Rows[i].Cells[3].Value.ToString() + " - "
                           ...
                           + "@" + DgvStockReceipt.Rows[i].Cells[22].Value.ToString() + "~";
            }
            First += Remarks;
            return First;
        }
        ...
        private void FillPurchases(List<VoucherDetailEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    DgvStockReceipt.Rows[i].Cells[19].Value = CommonFunctions.RemoveTrailingZeros(List[i].Discount) + "%";
                    DgvStockReceipt.Rows[i].Cells[20].Value = List[i].DiscountAmount;
                    DgvStockReceipt.Rows[i].Cells[21].Value = List[i].FlatDiscount;
                    DgvStockReceipt.Rows[i].Cells[22].Value = List[i].NetAmount;
